...
import com.widgetworld.core.widget.render.glance.render.GlanceCheckBox

internal class CheckBoxNode : RenderNode {
    @Composable
    override fun render(
        node: WidgetNode,
        ...
        if (node.widgetType == WidgetType.WIDGET_TYPE_GLANCE) {
            GlanceCheckBox.render(node, context, renderer)
        } else {
