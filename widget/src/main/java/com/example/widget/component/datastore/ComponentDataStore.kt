package com.example.widget.component.datastore
...
/**
 * Repository 패턴을 위한 베이스 클래스
 * 
 * ComponentDataStore를 래핑하여 추가적인 비즈니스 로직을 제공할 수 있습니다.
 * 
 * @param T 데이터 타입
 * @param dataStore ComponentDataStore 인스턴스
 */
abstract class ComponentDataStoreRepository<T>(
    protected val dataStore: ComponentDataStore<T>
) {
    /**
     * 데이터를 저장합니다.
     */
    open suspend fun save(context: Context, data: T) {
        dataStore.saveData(context, data)
    }
    
    /**
     * 데이터를 불러옵니다.
     */
    open suspend fun load(context: Context): T {
        return dataStore.loadData(context)
    }
    
    /**
     * 데이터를 삭제합니다.
     */
    open suspend fun clear(context: Context) {
        dataStore.clearData(context)
    }
}
